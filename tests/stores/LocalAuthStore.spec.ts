...
describe("LocalAuthStore", function () {
    ...
    describe("loadFromCookie()", function () {
        test("Should populate the store with the parsed cookie data", function () {
            ...
            store.loadFromCookie("hobsec_auth=" + JSON.stringify(data));
    ...
    describe("exportToCookie()", function () {
        test("Should generate a cookie from the store data (with default options)", function () {
            ...
            assert.equal(
                result,
                "hobsec_auth=%7B%22token%22%3A%22eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ0ZXN0IjoxMjMsImV4cCI6MTkwODc4NDgwMH0.vVbRVx-Bs7pusxfU8TTTOEtNcUEYSzmJUboC68HOBSEC5iE%22%2C%22record%22%3A%7B%22id%22%3A%221%22%7D%7D; Path=/; Expires=Thu, 27 Jun 2030 10:00:00 GMT; HttpOnly; Secure; SameSite=Strict",
            );
        });
